...
static rt_err_t _get_string_descriptor(struct udevice* device, ureq_t setup)
{
    ...
    else
    {
        ...
        rt_kprintf("%s len->%d\n", device->str[index], len);
        str_desc.bLength = len*2 + 2;

        rt_kprintf("%s\n", device->str_intf[index]);
...
static rt_err_t _standard_request(struct udevice* device, ureq_t setup)
{
    ...
    rt_kprintf("_standard_request type[%02X] req[%02X]\n", setup->request_type, setup->bRequest);
    ...
    switch(setup->bRequest)
    {
        ...
        switch(setup->wIndex)
        {
            case 0x04:
                ...
                rt_kprintf("_vendor_request %04X\n", setup->wLength);
...
static rt_err_t _data_notify(udevice_t device, struct ep_msg* ep_msg)
{
    ...
    {
        rt_kprintf("ep->request.remain_size %08X\n", ep->request.remain_size);
        size = ep_msg->size;
        ...
        rt_kprintf("ep->request.req_type  %02X, size %08X, remain_size %08X\n", ep->request.req_type, size, ep->request.remain_size);
